...
# Naive Model:
fit_naive_interaction <- glm(physical ~ discrim + I(age-74)  + male + black + med_group + black*discrim , data = physical_baseline ); lmCI(fit_naive_interaction)

linear_contrast_naive <- linContr.glm( model = fit_naive_interaction,
                                       contr.names = c( "discrim", "discrim:blackNon-Hispanic Black" ), 
                                       contr.coef = c(1,1), 
                                       transform = FALSE); linear_contrast_naive

#number of coeffience in our model 
num_coef <- length(coef(fit_naive_interaction))
...
rslt <- matrix(NA, nrow = num_replicates, ncol = num_coef + 1  )

for(i in 1:num_replicates){
  # random select observation 
  indices <- sample(num_observation, replace = TRUE  )
  subset_data <- physical_baseline[-indices,]
  fit <- glm(physical ~ discrim + I(age-74)  + male + black + med_group + black*discrim , data = subset_data, family = gaussian )
  linear_contrast <- linContr.glm( model = fit,
                                   contr.names = c( "discrim", "discrim:blackNon-Hispanic Black" ), 
                                   contr.coef = c(1,1), 
                                   transform = FALSE)
  rslt[i,] <- c(coef(fit),linear_contrast$Est  )
}

rslt_table <- data.frame(covarite = c( rownames(data.frame(lmCI(fit_naive_interaction))), "beta_1 + beta_3" ) , 
...
fileout <- "/Users/Esteban/Desktop/Schools/UCI/Qualifiy_exams/Year-2/Data\ Analysis/qual_2023/Latex/mod1c.tex"
